...
nucleo.DEBUG=true;if(typeof(jQuery)=="undefined"){nucleo.jquery=false;console.warn("JQuery is not available")}else{nucleo.jquery=true;console.info("JQuery is available")}nucleo.ajax=function(d){if(nucleo.jquery){$.ajax(d);return}var a,c=new XMLHttpRequest();c.open(d.type,d.url,true);if(d.mimeType!==undefined){c.overrideMimeType(d.mimeType)}c.onreadystatechange=function(){if(c.readyState==4){if(d.dataType==="json"){a=JSON.parse(c.responseText)}else{a=c.responseText}d.success(a)}};c.onerror=function(){d.error(c,c.status,c.statusText)
};c.send()};nucleo.util={piOver2:Math.PI/2,deg2rad:Math.PI/180,rad2deg:180/Math.PI,isMac:function(){return navigator.platform.toUpperCase().indexOf("MAC")!=-1},int2rgb:function(a){return[((a>>16)&255)/256,((a>>8)&255)/256,(a&255)/256]},frac2rgb:function(e,d,a){var f=nucleo.util.createArr3(e,d,a);f[0]=Math.round(255*f[0]);f[1]=Math.round(255*f[1]);f[2]=Math.round(255*f[2]);return f},rgb2frac:function(e,d,a){var f=nucleo.util.createArr3(e,d,a);f[0]=Math.round(f[0]/255);f[1]=Math.round(f[1]/255);f[2]=Math.round(f[2]/255);
return f},rgb2hex:function(e,d,a){var f=nucleo.util.createArr3(e,d,a);return"#"+((1<<24)+(f[0]<<16)+(f[1]<<8)+f[2]).toString(16).slice(1)},hex2rgb:function(d){var c=/^#?([a-f\d])([a-f\d])([a-f\d])$/i;d=d.replace(c,function(f,j,h,e){return j+j+h+h+e+e});var a=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(d);return a?[parseInt(a[1],16),parseInt(a[2],16),parseInt(a[3],16)]:null},rgb2decimal:function(a){if(a==null||a==undefined){return null}return[a[0]/255,a[1]/255,a[2]/255]},createColor:function(f,e,a){var d=[];
if(f==undefined){return null}if(f instanceof Array){var h=f.slice(0);f=h[0];e=h[1];a=h[2]}if(typeof(f)=="string"){d=this.rgb2decimal(this.hex2rgb(f))}else{if(typeof(f)=="number"){if(f<0||e==undefined||a==undefined||e<0||a<0){return null}else{if(f>1||e>1||a>1){d=this.rgb2decimal([f,e,a])}else{d=[f,e,a]}}}}return d},format:function(c,e){var f=Math.pow(10,e);if(typeof(c)=="object"){var a="[";for(var d=0;d<c.length-1;d+=1){a+=Math.round(c[d]*f)/f+", "}a+=Math.round(c[c.length-1]*f)/f+"]"}else{if(typeof(c)=="number"){a="["+Math.round(c*f)/f+"]"
}}return a},createVec3:function(a,e,d){var c=vec3.create();if(a instanceof Array||a instanceof Float32Array){c=vec3.clone(a)}else{c=vec3.fromValues(a,e,d)}return c},createArr3:function(a,e,d){var c=[];if(a instanceof Array||a instanceof Float32Array){c[0]=a[0];c[1]=a[1];c[2]=a[2]}else{c[0]=a;c[1]=e;c[2]=d}return c},generateUID:function(){function a(){return(((1+Math.random())*65536)|0).toString(16).substring(1)}return(a()+"-"+a()+"-"+a()+"-"+a())},extend:function(){var c={};var a=arguments.length;
for(var d=0;d<a;d++){for(p in arguments[d]){if(arguments[d].hasOwnProperty(p)){c[p]=arguments[d][p]}}}return c},getPath:function(a){if(a==undefined||a==null){return""}else{if(a.length-1==a.lastIndexOf("/")){return a}else{if(a.lastIndexOf(".")>a.lastIndexOf("/")){return a.substring(0,a.lastIndexOf("/")+1)}else{return a+"/"}}}},getAngle:function(a){if(a>360||a<-360){return a%360}else{return a}},deg2rad:function(a){return a*Math.PI/180},doTimer:function(c,f,l,e){var j=(c/100)*(f/10),d=c/j,h=0,a=new Date().getTime();
...
})();nucleo.Notifier=function(){this.targetList={};this.sourceList={}};nucleo.Notifier.prototype.subscribe=function(d,c){if(typeof(d)=="string"){this._addTarget(d,c)}else{if(d instanceof Array){for(var a=0;a<d.length;a+=1){this._addTarget(d[a],c)}}else{throw"Notifier.receives: this method receives a string or a list of strings"}}};nucleo.Notifier.prototype.publish=function(d,c){if(typeof(d)=="string"){this._addSource(d,c)}else{if(d instanceof Array){for(var a=0;a<d.length;a+=1){this._addSource(d[a],c)
}}else{throw"Notifier.sends: this method receives a string or a list of strings"}}};nucleo.Notifier.prototype._addTarget=function(a,c){nucleo.util.console("Notifier: adding target for event "+a);var d=this.targetList;if(d[a]==undefined){d[a]=[]}d[a].push(c)};nucleo.Notifier.prototype._addSource=function(c,e){nucleo.util.console("Notifier: adding source for event "+c);var d=this.targetList;var a=this.sourceList;if(a[c]==undefined){a[c]=[]}if(d[c]==undefined){d[c]=[]}a[c].push(e)};nucleo.Notifier.prototype.fire=function(d,e){if(this.targetList[d].length==0){return
}var a=this;function c(){var k=a.targetList;var f=a.sourceList[d].indexOf(e);if(f==-1){throw"The source "+e+" is not registered to trigger the event "+d+". Did you use Notifier.publish?"}nucleo.util.console("Notifier: firing "+d);var h=a.targetList[d];for(var j=0;j<h.length;j++){if(typeof(h[j])=="object"){h[j].handleEvent(d,e)}else{if(typeof(h[j]=="function")){h[j](d,e)}}}}setTimeout(function(){c()},0)};nucleo.Notifier.prototype.getEvents=function(){var c=[];for(var a in this.sourceList){c.push(a)
}return c};nucleo.Notifier.prototype.getTargetsFor=function(d){var a=this.targetList[d];var e=[];for(var c=0;c<a.length;c++){e.push(a[c])}return e};nucleo.Notifier.instance=new nucleo.Notifier();(function(c){var a={};if(typeof(exports)==="undefined"){if(typeof define=="function"&&typeof define.amd=="object"&&define.amd){a.exports={};define(function(){return a.exports})}else{a.exports=typeof(window)!=="undefined"?window:c}}else{a.exports=exports}(function(n){if(!u){var u=0.000001}if(!f){var f=(typeof Float32Array!=="undefined")?Float32Array:Array
}if(!l){var l=Math.random}var h={};h.setMatrixArrayType=function(v){f=v};if(typeof(n)!=="undefined"){n.glMatrix=h}var m=Math.PI/180;h.toRadian=function(v){return v*m};var s={};s.create=function(){var v=new f(2);v[0]=0;v[1]=0;return v};s.clone=function(v){var w=new f(2);w[0]=v[0];w[1]=v[1];return w};s.fromValues=function(v,z){var w=new f(2);w[0]=v;w[1]=z;return w};s.copy=function(w,v){w[0]=v[0];w[1]=v[1];return w};s.set=function(w,v,z){w[0]=v;w[1]=z;return w};s.add=function(x,w,v){x[0]=w[0]+v[0];x[1]=w[1]+v[1];
return x};s.subtract=function(x,w,v){x[0]=w[0]-v[0];x[1]=w[1]-v[1];return x};s.sub=s.subtract;s.multiply=function(x,w,v){x[0]=w[0]*v[0];x[1]=w[1]*v[1];return x};s.mul=s.multiply;s.divide=function(x,w,v){x[0]=w[0]/v[0];x[1]=w[1]/v[1];return x};s.div=s.divide;s.min=function(x,w,v){x[0]=Math.min(w[0],v[0]);x[1]=Math.min(w[1],v[1]);return x};s.max=function(x,w,v){x[0]=Math.max(w[0],v[0]);x[1]=Math.max(w[1],v[1]);return x};s.scale=function(x,w,v){x[0]=w[0]*v;x[1]=w[1]*v;return x};s.scaleAndAdd=function(x,w,v,y){x[0]=w[0]+(v[0]*y);
x[1]=w[1]+(v[1]*y);return x};s.distance=function(z,w){var v=w[0]-z[0],A=w[1]-z[1];return Math.sqrt(v*v+A*A)};s.dist=s.distance;s.squaredDistance=function(z,w){var v=w[0]-z[0],A=w[1]-z[1];return v*v+A*A};s.sqrDist=s.squaredDistance;s.length=function(w){var v=w[0],z=w[1];return Math.sqrt(v*v+z*z)};s.len=s.length;s.squaredLength=function(w){var v=w[0],z=w[1];return v*v+z*z};s.sqrLen=s.squaredLength;s.negate=function(w,v){w[0]=-v[0];w[1]=-v[1];return w};s.normalize=function(A,z){var w=z[0],B=z[1];var v=w*w+B*B;
if(v>0){v=1/Math.sqrt(v);A[0]=z[0]*v;A[1]=z[1]*v}return A};s.dot=function(w,v){return w[0]*v[0]+w[1]*v[1]};s.cross=function(x,w,v){var y=w[0]*v[1]-w[1]*v[0];x[0]=x[1]=0;x[2]=y;return x};s.lerp=function(x,w,v,y){var A=w[0],z=w[1];x[0]=A+y*(v[0]-A);x[1]=z+y*(v[1]-z);return x};s.random=function(v,x){x=x||1;var w=l()*2*Math.PI;v[0]=Math.cos(w)*x;v[1]=Math.sin(w)*x;return v};s.transformMat2=function(A,z,w){var v=z[0],B=z[1];A[0]=w[0]*v+w[2]*B;A[1]=w[1]*v+w[3]*B;return A};s.transformMat2d=function(A,z,w){var v=z[0],B=z[1];
A[0]=w[0]*v+w[2]*B+w[4];A[1]=w[1]*v+w[3]*B+w[5];return A};s.transformMat3=function(A,z,w){var v=z[0],B=z[1];A[0]=w[0]*v+w[3]*B+w[6];A[1]=w[1]*v+w[4]*B+w[7];return A};s.transformMat4=function(A,z,w){var v=z[0],B=z[1];A[0]=w[0]*v+w[4]*B+w[12];A[1]=w[1]*v+w[5]*B+w[13];return A};s.forEach=(function(){var v=s.create();return function(y,C,D,B,A,w){var z,x;if(!C){C=2}if(!D){D=0}if(B){x=Math.min((B*C)+D,y.length)}else{x=y.length}for(z=D;z<x;z+=C){v[0]=y[z];v[1]=y[z+1];A(v,v,w);y[z]=v[0];y[z+1]=v[1]}return y
}})();s.str=function(v){return"vec2("+v[0]+", "+v[1]+")"};if(typeof(n)!=="undefined"){n.vec2=s}var q={};q.create=function(){var v=new f(3);v[0]=0;v[1]=0;v[2]=0;return v};q.clone=function(v){var w=new f(3);w[0]=v[0];w[1]=v[1];w[2]=v[2];return w};q.fromValues=function(v,B,A){var w=new f(3);w[0]=v;w[1]=B;w[2]=A;return w};q.copy=function(w,v){w[0]=v[0];w[1]=v[1];w[2]=v[2];return w};q.set=function(w,v,B,A){w[0]=v;w[1]=B;w[2]=A;return w};q.add=function(x,w,v){x[0]=w[0]+v[0];x[1]=w[1]+v[1];x[2]=w[2]+v[2];
return x};q.subtract=function(x,w,v){x[0]=w[0]-v[0];x[1]=w[1]-v[1];x[2]=w[2]-v[2];return x};q.sub=q.subtract;q.multiply=function(x,w,v){x[0]=w[0]*v[0];x[1]=w[1]*v[1];x[2]=w[2]*v[2];return x};q.mul=q.multiply;q.divide=function(x,w,v){x[0]=w[0]/v[0];x[1]=w[1]/v[1];x[2]=w[2]/v[2];return x};q.div=q.divide;q.min=function(x,w,v){x[0]=Math.min(w[0],v[0]);x[1]=Math.min(w[1],v[1]);x[2]=Math.min(w[2],v[2]);return x};q.max=function(x,w,v){x[0]=Math.max(w[0],v[0]);x[1]=Math.max(w[1],v[1]);x[2]=Math.max(w[2],v[2]);
return x};q.scale=function(x,w,v){x[0]=w[0]*v;x[1]=w[1]*v;x[2]=w[2]*v;return x};q.scaleAndAdd=function(x,w,v,y){x[0]=w[0]+(v[0]*y);x[1]=w[1]+(v[1]*y);x[2]=w[2]+(v[2]*y);return x};q.distance=function(A,w){var v=w[0]-A[0],C=w[1]-A[1],B=w[2]-A[2];return Math.sqrt(v*v+C*C+B*B)};q.dist=q.distance;q.squaredDistance=function(A,w){var v=w[0]-A[0],C=w[1]-A[1],B=w[2]-A[2];return v*v+C*C+B*B};q.sqrDist=q.squaredDistance;q.length=function(w){var v=w[0],B=w[1],A=w[2];return Math.sqrt(v*v+B*B+A*A)};q.len=q.length;
q.squaredLength=function(w){var v=w[0],B=w[1],A=w[2];return v*v+B*B+A*A};q.sqrLen=q.squaredLength;q.negate=function(w,v){w[0]=-v[0];w[1]=-v[1];w[2]=-v[2];return w};q.normalize=function(B,A){var w=A[0],D=A[1],C=A[2];var v=w*w+D*D+C*C;if(v>0){v=1/Math.sqrt(v);B[0]=A[0]*v;B[1]=A[1]*v;B[2]=A[2]*v}return B};q.dot=function(w,v){return w[0]*v[0]+w[1]*v[1]+w[2]*v[2]};q.cross=function(w,B,A){var v=B[0],D=B[1],C=B[2],z=A[0],y=A[1],x=A[2];w[0]=D*x-C*y;w[1]=C*z-v*x;w[2]=v*y-D*z;return w};q.lerp=function(x,w,v,y){var B=w[0],A=w[1],z=w[2];
x[0]=B+y*(v[0]-B);x[1]=A+y*(v[1]-A);x[2]=z+y*(v[2]-z);return x};q.random=function(v,A){A=A||1;var x=l()*2*Math.PI;var y=(l()*2)-1;var w=Math.sqrt(1-y*y)*A;v[0]=Math.cos(x)*w;v[1]=Math.sin(x)*w;v[2]=y*A;return v};q.transformMat4=function(B,A,w){var v=A[0],D=A[1],C=A[2];B[0]=w[0]*v+w[4]*D+w[8]*C+w[12];B[1]=w[1]*v+w[5]*D+w[9]*C+w[13];B[2]=w[2]*v+w[6]*D+w[10]*C+w[14];return B};q.transformMat3=function(B,A,w){var v=A[0],D=A[1],C=A[2];B[0]=v*w[0]+D*w[3]+C*w[6];B[1]=v*w[1]+D*w[4]+C*w[7];B[2]=v*w[2]+D*w[5]+C*w[8];
return B};q.transformQuat=function(E,K,v){var L=K[0],J=K[1],I=K[2],G=v[0],F=v[1],D=v[2],H=v[3],B=H*L+F*I-D*J,A=H*J+D*L-G*I,w=H*I+G*J-F*L,C=-G*L-F*J-D*I;E[0]=B*H+C*-G+A*-D-w*-F;E[1]=A*H+C*-F+w*-G-B*-D;E[2]=w*H+C*-D+B*-F-A*-G;return E};q.rotateX=function(x,w,v,A){var z=[],y=[];z[0]=w[0]-v[0];z[1]=w[1]-v[1];z[2]=w[2]-v[2];y[0]=z[0];y[1]=z[1]*Math.cos(A)-z[2]*Math.sin(A);y[2]=z[1]*Math.sin(A)+z[2]*Math.cos(A);x[0]=y[0]+v[0];x[1]=y[1]+v[1];x[2]=y[2]+v[2];return x};q.rotateY=function(x,w,v,A){var z=[],y=[];
z[0]=w[0]-v[0];z[1]=w[1]-v[1];z[2]=w[2]-v[2];y[0]=z[2]*Math.sin(A)+z[0]*Math.cos(A);y[1]=z[1];y[2]=z[2]*Math.cos(A)-z[0]*Math.sin(A);x[0]=y[0]+v[0];x[1]=y[1]+v[1];x[2]=y[2]+v[2];return x};q.rotateZ=function(x,w,v,A){var z=[],y=[];z[0]=w[0]-v[0];z[1]=w[1]-v[1];z[2]=w[2]-v[2];y[0]=z[0]*Math.cos(A)-z[1]*Math.sin(A);y[1]=z[0]*Math.sin(A)+z[1]*Math.cos(A);y[2]=z[2];x[0]=y[0]+v[0];x[1]=y[1]+v[1];x[2]=y[2]+v[2];return x};q.forEach=(function(){var v=q.create();return function(y,C,D,B,A,w){var z,x;if(!C){C=3
}if(!D){D=0}if(B){x=Math.min((B*C)+D,y.length)}else{x=y.length}for(z=D;z<x;z+=C){v[0]=y[z];v[1]=y[z+1];v[2]=y[z+2];A(v,v,w);y[z]=v[0];y[z+1]=v[1];y[z+2]=v[2]}return y}})();q.str=function(v){return"vec3("+v[0]+", "+v[1]+", "+v[2]+")"};if(typeof(n)!=="undefined"){n.vec3=q}var o={};o.create=function(){var v=new f(4);v[0]=0;v[1]=0;v[2]=0;v[3]=0;return v};o.clone=function(v){var w=new f(4);w[0]=v[0];w[1]=v[1];w[2]=v[2];w[3]=v[3];return w};o.fromValues=function(v,D,C,A){var B=new f(4);B[0]=v;B[1]=D;B[2]=C;
B[3]=A;return B};o.copy=function(w,v){w[0]=v[0];w[1]=v[1];w[2]=v[2];w[3]=v[3];return w};o.set=function(B,v,D,C,A){B[0]=v;B[1]=D;B[2]=C;B[3]=A;return B};o.add=function(x,w,v){x[0]=w[0]+v[0];x[1]=w[1]+v[1];x[2]=w[2]+v[2];x[3]=w[3]+v[3];return x};o.subtract=function(x,w,v){x[0]=w[0]-v[0];x[1]=w[1]-v[1];x[2]=w[2]-v[2];x[3]=w[3]-v[3];return x};o.sub=o.subtract;o.multiply=function(x,w,v){x[0]=w[0]*v[0];x[1]=w[1]*v[1];x[2]=w[2]*v[2];x[3]=w[3]*v[3];return x};o.mul=o.multiply;o.divide=function(x,w,v){x[0]=w[0]/v[0];
x[1]=w[1]/v[1];x[2]=w[2]/v[2];x[3]=w[3]/v[3];return x};o.div=o.divide;o.min=function(x,w,v){x[0]=Math.min(w[0],v[0]);x[1]=Math.min(w[1],v[1]);x[2]=Math.min(w[2],v[2]);x[3]=Math.min(w[3],v[3]);return x};o.max=function(x,w,v){x[0]=Math.max(w[0],v[0]);x[1]=Math.max(w[1],v[1]);x[2]=Math.max(w[2],v[2]);x[3]=Math.max(w[3],v[3]);return x};o.scale=function(x,w,v){x[0]=w[0]*v;x[1]=w[1]*v;x[2]=w[2]*v;x[3]=w[3]*v;return x};o.scaleAndAdd=function(x,w,v,y){x[0]=w[0]+(v[0]*y);x[1]=w[1]+(v[1]*y);x[2]=w[2]+(v[2]*y);
x[3]=w[3]+(v[3]*y);return x};o.distance=function(C,A){var v=A[0]-C[0],E=A[1]-C[1],D=A[2]-C[2],B=A[3]-C[3];return Math.sqrt(v*v+E*E+D*D+B*B)};o.dist=o.distance;o.squaredDistance=function(C,A){var v=A[0]-C[0],E=A[1]-C[1],D=A[2]-C[2],B=A[3]-C[3];return v*v+E*E+D*D+B*B};o.sqrDist=o.squaredDistance;o.length=function(B){var v=B[0],D=B[1],C=B[2],A=B[3];return Math.sqrt(v*v+D*D+C*C+A*A)};o.len=o.length;o.squaredLength=function(B){var v=B[0],D=B[1],C=B[2],A=B[3];return v*v+D*D+C*C+A*A};o.sqrLen=o.squaredLength;
o.negate=function(w,v){w[0]=-v[0];w[1]=-v[1];w[2]=-v[2];w[3]=-v[3];return w};o.normalize=function(D,C){var A=C[0],F=C[1],E=C[2],B=C[3];var v=A*A+F*F+E*E+B*B;if(v>0){v=1/Math.sqrt(v);D[0]=C[0]*v;D[1]=C[1]*v;D[2]=C[2]*v;D[3]=C[3]*v}return D};o.dot=function(w,v){return w[0]*v[0]+w[1]*v[1]+w[2]*v[2]+w[3]*v[3]};o.lerp=function(x,w,v,y){var B=w[0],A=w[1],z=w[2],C=w[3];x[0]=B+y*(v[0]-B);x[1]=A+y*(v[1]-A);x[2]=z+y*(v[2]-z);x[3]=C+y*(v[3]-C);return x};o.random=function(v,w){w=w||1;v[0]=l();v[1]=l();v[2]=l();
v[3]=l();o.normalize(v,v);o.scale(v,v,w);return v};o.transformMat4=function(D,C,A){var v=C[0],F=C[1],E=C[2],B=C[3];D[0]=A[0]*v+A[4]*F+A[8]*E+A[12]*B;D[1]=A[1]*v+A[5]*F+A[9]*E+A[13]*B;D[2]=A[2]*v+A[6]*F+A[10]*E+A[14]*B;D[3]=A[3]*v+A[7]*F+A[11]*E+A[15]*B;return D};o.transformQuat=function(E,K,v){var L=K[0],J=K[1],I=K[2],G=v[0],F=v[1],D=v[2],H=v[3],B=H*L+F*I-D*J,A=H*J+D*L-G*I,w=H*I+G*J-F*L,C=-G*L-F*J-D*I;E[0]=B*H+C*-G+A*-D-w*-F;E[1]=A*H+C*-F+w*-G-B*-D;E[2]=w*H+C*-D+B*-F-A*-G;return E};o.forEach=(function(){var v=o.create();
return function(y,C,D,B,A,w){var z,x;if(!C){C=4}if(!D){D=0}if(B){x=Math.min((B*C)+D,y.length)}else{x=y.length}for(z=D;z<x;z+=C){v[0]=y[z];v[1]=y[z+1];v[2]=y[z+2];v[3]=y[z+3];A(v,v,w);y[z]=v[0];y[z+1]=v[1];y[z+2]=v[2];y[z+3]=v[3]}return y}})();o.str=function(v){return"vec4("+v[0]+", "+v[1]+", "+v[2]+", "+v[3]+")"};if(typeof(n)!=="undefined"){n.vec4=o}var j={};j.create=function(){var v=new f(4);v[0]=1;v[1]=0;v[2]=0;v[3]=1;return v};j.clone=function(v){var w=new f(4);w[0]=v[0];w[1]=v[1];w[2]=v[2];w[3]=v[3];
return w};j.copy=function(w,v){w[0]=v[0];w[1]=v[1];w[2]=v[2];w[3]=v[3];return w};j.identity=function(v){v[0]=1;v[1]=0;v[2]=0;v[3]=1;return v};j.transpose=function(x,w){if(x===w){var v=w[1];x[1]=w[2];x[2]=v}else{x[0]=w[0];x[1]=w[2];x[2]=w[1];x[3]=w[3]}return x};j.invert=function(z,x){var y=x[0],w=x[1],v=x[2],B=x[3],A=y*B-v*w;if(!A){return null}A=1/A;z[0]=B*A;z[1]=-w*A;z[2]=-v*A;z[3]=y*A;return z};j.adjoint=function(x,v){var w=v[0];x[0]=v[3];x[1]=-v[1];x[2]=-v[2];x[3]=w;return x};j.determinant=function(v){return v[0]*v[3]-v[2]*v[1]
};j.multiply=function(z,E,C){var y=E[0],x=E[1],w=E[2],v=E[3];var F=C[0],D=C[1],B=C[2],A=C[3];z[0]=y*F+w*D;z[1]=x*F+v*D;z[2]=y*B+w*A;z[3]=x*B+v*A;return z};j.mul=j.multiply;j.rotate=function(z,C,B){var y=C[0],x=C[1],w=C[2],v=C[3],D=Math.sin(B),A=Math.cos(B);z[0]=y*A+w*D;z[1]=x*A+v*D;z[2]=y*-D+w*A;z[3]=x*-D+v*A;return z};j.scale=function(A,B,D){var z=B[0],y=B[1],x=B[2],w=B[3],E=D[0],C=D[1];A[0]=z*E;A[1]=y*E;A[2]=x*C;A[3]=w*C;return A};j.str=function(v){return"mat2("+v[0]+", "+v[1]+", "+v[2]+", "+v[3]+")"
};j.frob=function(v){return(Math.sqrt(Math.pow(v[0],2)+Math.pow(v[1],2)+Math.pow(v[2],2)+Math.pow(v[3],2)))};j.LDU=function(v,y,x,w){v[2]=w[2]/w[0];x[0]=w[0];x[1]=w[1];x[3]=w[3]-v[2]*x[1];return[v,y,x]};if(typeof(n)!=="undefined"){n.mat2=j}var t={};t.create=function(){var v=new f(6);v[0]=1;v[1]=0;v[2]=0;v[3]=1;v[4]=0;v[5]=0;return v};t.clone=function(v){var w=new f(6);w[0]=v[0];w[1]=v[1];w[2]=v[2];w[3]=v[3];w[4]=v[4];w[5]=v[5];return w};t.copy=function(w,v){w[0]=v[0];w[1]=v[1];w[2]=v[2];w[3]=v[3];
w[4]=v[4];w[5]=v[5];return w};t.identity=function(v){v[0]=1;v[1]=0;v[2]=0;v[3]=1;v[4]=0;v[5]=0;return v};t.invert=function(w,A){var v=A[0],D=A[1],C=A[2],B=A[3],y=A[4],x=A[5];var z=v*B-D*C;if(!z){return null}z=1/z;w[0]=B*z;w[1]=-D*z;w[2]=-C*z;w[3]=v*z;w[4]=(C*x-B*y)*z;w[5]=(D*y-v*x)*z;return w};t.determinant=function(v){return v[0]*v[3]-v[1]*v[2]};t.multiply=function(z,G,E){var y=G[0],x=G[1],w=G[2],v=G[3],J=G[4],I=G[5],H=E[0],F=E[1],D=E[2],C=E[3],B=E[4],A=E[5];z[0]=y*H+w*F;z[1]=x*H+v*F;z[2]=y*D+w*C;
z[3]=x*D+v*C;z[4]=y*B+w*A+J;z[5]=x*B+v*A+I;return z};t.mul=t.multiply;t.rotate=function(z,C,B){var y=C[0],x=C[1],w=C[2],v=C[3],F=C[4],D=C[5],E=Math.sin(B),A=Math.cos(B);z[0]=y*A+w*E;z[1]=x*A+v*E;z[2]=y*-E+w*A;z[3]=x*-E+v*A;z[4]=F;z[5]=D;return z};t.scale=function(A,B,E){var z=B[0],y=B[1],x=B[2],w=B[3],G=B[4],F=B[5],D=E[0],C=E[1];A[0]=z*D;A[1]=y*D;A[2]=x*C;A[3]=w*C;A[4]=G;A[5]=F;return A};t.translate=function(A,B,E){var z=B[0],y=B[1],x=B[2],w=B[3],G=B[4],F=B[5],D=E[0],C=E[1];A[0]=z;A[1]=y;A[2]=x;A[3]=w;
A[4]=z*D+x*C+G;A[5]=y*D+w*C+F;return A};t.str=function(v){return"mat2d("+v[0]+", "+v[1]+", "+v[2]+", "+v[3]+", "+v[4]+", "+v[5]+")"};t.frob=function(v){return(Math.sqrt(Math.pow(v[0],2)+Math.pow(v[1],2)+Math.pow(v[2],2)+Math.pow(v[3],2)+Math.pow(v[4],2)+Math.pow(v[5],2)+1))};if(typeof(n)!=="undefined"){n.mat2d=t}var e={};e.create=function(){var v=new f(9);v[0]=1;v[1]=0;v[2]=0;v[3]=0;v[4]=1;v[5]=0;v[6]=0;v[7]=0;v[8]=1;return v};e.fromMat4=function(w,v){w[0]=v[0];w[1]=v[1];w[2]=v[2];w[3]=v[4];w[4]=v[5];
w[5]=v[6];w[6]=v[8];w[7]=v[9];w[8]=v[10];return w};e.clone=function(v){var w=new f(9);w[0]=v[0];w[1]=v[1];w[2]=v[2];w[3]=v[3];w[4]=v[4];w[5]=v[5];w[6]=v[6];w[7]=v[7];w[8]=v[8];return w};e.copy=function(w,v){w[0]=v[0];w[1]=v[1];w[2]=v[2];w[3]=v[3];w[4]=v[4];w[5]=v[5];w[6]=v[6];w[7]=v[7];w[8]=v[8];return w};e.identity=function(v){v[0]=1;v[1]=0;v[2]=0;v[3]=0;v[4]=1;v[5]=0;v[6]=0;v[7]=0;v[8]=1;return v};e.transpose=function(x,w){if(x===w){var z=w[1],y=w[2],v=w[5];x[1]=w[3];x[2]=w[6];x[3]=z;x[5]=w[7];
x[6]=y;x[7]=v}else{x[0]=w[0];x[1]=w[3];x[2]=w[6];x[3]=w[1];x[4]=w[4];x[5]=w[7];x[6]=w[2];x[7]=w[5];x[8]=w[8]}return x};e.invert=function(z,G){var y=G[0],x=G[1],w=G[2],J=G[3],I=G[4],H=G[5],E=G[6],D=G[7],B=G[8],A=B*I-H*D,v=-B*J+H*E,F=D*J-I*E,C=y*A+x*v+w*F;if(!C){return null}C=1/C;z[0]=A*C;z[1]=(-B*x+w*D)*C;z[2]=(H*x-w*I)*C;z[3]=v*C;z[4]=(B*y-w*E)*C;z[5]=(-H*y+w*J)*C;z[6]=F*C;z[7]=(-D*y+x*E)*C;z[8]=(I*y-x*J)*C;return z};e.adjoint=function(y,C){var x=C[0],w=C[1],v=C[2],F=C[3],E=C[4],D=C[5],B=C[6],A=C[7],z=C[8];
...
};nucleo.Camera.prototype.getAzimuth=function(){return this._azimuth};nucleo.Camera.prototype.setElevation=function(a){this._elevation=this._getAngle(a);this._computeMatrix();this._getAxes();if(this.type==nucleo.Camera.TYPE.ORBITING||this.type==nucleo.Camera.TYPE.EXPLORING){this._getPosition()}else{if(this.type==nucleo.Camera.TYPE.TRACKING){this._getFocalPoint()}}return this};nucleo.Camera.prototype.setRoll=function(c){var a=nucleo.Camera;this._roll=this._getAngle(c);this._computeMatrix();this._getAxes();
if(this.type==a.TYPE.ORBITING||this.type==a.TYPE.EXPLORING){this._getPosition()}else{if(this.type==a.TYPE.TRACKING){this._getFocalPoint()}}return this};nucleo.Camera.prototype.rotate=function(l,k,c){var j=nucleo.Camera;if(this.type==j.TYPE.EXPLORING){l=this._getAngle(l);k=this._getAngle(k);c=this._getAngle(c);var e=quat.setAxisAngle(quat.create(),[1,0,0],-k*nucleo.define.DEG_2_RAD);var d=quat.setAxisAngle(quat.create(),[0,1,0],-l*nucleo.define.DEG_2_RAD);if(this._rotate_world){e=quat.setAxisAngle(quat.create(),[1,0,0],k*nucleo.define.DEG_2_RAD);
d=quat.setAxisAngle(quat.create(),[0,1,0],l*nucleo.define.DEG_2_RAD)}var a=quat.setAxisAngle(quat.create(),[0,0,1],c*nucleo.define.DEG_2_RAD);var h=quat.multiply(quat.create(),d,e);h=quat.multiply(quat.create(),h,a);var f=mat4.fromQuat(mat4.create(),h);mat4.translate(this._matrix,this._matrix,[0,0,-this._distance]);mat4.multiply(this._matrix,this._matrix,f);mat4.translate(this._matrix,this._matrix,[0,0,this._distance])}else{if(Math.abs(this._elevation+k)>90){return}this._relElevation=this._getAngle(k);
this._relAzimuth=this._getAngle(l);this._relRoll=this._getAngle(c);this._elevation+=this._relElevation;this._azimuth+=this._relAzimuth;this._roll+=this._relRoll;this._computeMatrix()}this._getAxes();if(this.type==j.TYPE.ORBITING||this.type==j.TYPE.EXPLORING){this._getPosition()}else{if(this.type==j.TYPE.TRACKING){this._getFocalPoint()}}this._update();return this};nucleo.Camera.prototype.dolly=function(d){var a=nucleo.Camera;var f=this._forward;var e=vec3.clone(this._position);var c=d*this._dollyingStep;
e[0]+=c*f[0];e[1]+=c*f[1];e[2]+=c*f[2];this._setPosition(e);if(this.type==a.TYPE.ORBITING||this.type==a.TYPE.EXPLORING){this._getDistance()}else{if(this.type==a.TYPE.TRACKING){vec3.add(this._focalPoint,e,this._distanceVector)}}return this};nucleo.Camera.prototype.pan=function(c,a){var d=nucleo.util.createVec3(c,a,0);var e=vec3.clone(this._position);vec3.add(e,e,vec3.scale(vec3.create(),this._right,d[0]));vec3.add(e,e,vec3.scale(vec3.create(),this._up,d[1]));this._setPosition(e);return this};nucleo.Camera.prototype.translate=function(a,f,d){var c=nucleo.util.createVec3(a,f,d);
...
this.setLookupTable(this.lutID)}}};nucleo.Scene.prototype.setLoadingMode=function(c){var a=nucleo.define.Model.LOADING_MODE;if(c===undefined||c===null||(c!==a.LIVE&&c!==a.LATER&&c!==a.DETACHED)){throw ("the mode "+c+"is not a valid loading mode")}this.loadingMode=c};nucleo.Scene.prototype._updateBoundingBoxWith=function(c){var a=c._bb;nucleo.util.console("Scene: updating metrics with ("+a[0]+","+a[1]+","+a[2]+") - ("+a[3]+","+a[4]+","+a[5]+")");if(this._actors.length==1){this.bb=this._actors[0]._bb.slice(0);
this.toys.update();return}var d=this.bb;var e=this.centre;d[0]=Math.min(d[0],a[0]);d[1]=Math.min(d[1],a[1]);d[2]=Math.min(d[2],a[2]);d[3]=Math.max(d[3],a[3]);d[4]=Math.max(d[4],a[4]);d[5]=Math.max(d[5],a[5]);e[0]=(d[3]+d[0])/2;e[1]=(d[4]+d[1])/2;e[2]=(d[5]+d[2])/2;e[0]=Math.round(e[0]*1000)/1000;e[1]=Math.round(e[1]*1000)/1000;e[2]=Math.round(e[2]*1000)/1000;this.toys.update()};nucleo.Scene.prototype.computeBoundingBox=function(){if(this._actors.length>0){this.bb=this._actors[0]._bb.slice(0)}else{this.bb=[0,0,0,0,0,0]
}this.centre=[0,0,0];var a=this._actors.length;while(a--){this._updateBoundingBoxWith(this._actors[a])}};nucleo.Scene.prototype.addActor=function(a){a.setScene(this);if(this.normalsFlipped){a.flipNormals(true)}if(this.lutID!=null){a.setLookupTable(this.lutID,this.scalarMIN,this.scalarMAX)}this._actors.push(a);this._updateBoundingBoxWith(a);nucleo.util.console("Scene: Actor for model "+a.model.name+" added");nucleo.Notifier.instance.fire(nucleo.EVENTS.SCENE_UPDATED,this)};nucleo.Scene.prototype.updateActor=function(c){if(!this.hasActor(c)){return
}c.dirty=true;var a=this.views.length;while(a--){this.views[a].renderer.reallocate()}c.dirty=false};nucleo.Scene.prototype.removeActor=function(d){var c=this._actors.indexOf(d);var a=this._actors.splice(c,1);a=null;this.computeBoundingBox()};nucleo.Scene.prototype.hasActor=function(c){if(c instanceof nucleo.Actor){return(this._actors.indexOf(c)!=-1)}else{if(typeof(c)=="string"){var a=this.getActorByName(c);return(a!=undefined)}else{return false}}};nucleo.Scene.prototype.setPropertyForAll=function(d,c){var a=this._actors.length;
while(a--){this._actors[a].setProperty(d,c)}};nucleo.Scene.prototype.setPropertyFor=function(f,e,d){var a=f.length;while(a--){if(this.hasActor(f[a])){var c=undefined;if(typeof(f[a])=="string"){c=this.getActorByName(f[a])}else{if(f[a] instanceof nucleo.Actor){f[a].setProperty(e,d)}else{throw"Scene.setPropertyFor: ERROR, the list of actors is invalid"}}}}};nucleo.Scene.prototype.updateScalarRange=function(){var a=this._actors.length;while(a--){var c=this._actors[a];if(c.model.scalars&&c.model.scalars.max()>this.scalarMAX){this.scalarMAX=c.model.scalars.max()
}if(c.model.scalars&&c.model.scalars.min()<this.scalarMIN){this.scalarMIN=c.model.scalars.min()}}};nucleo.Scene.prototype.setLookupTable=function(c){this.lutID=c;var a=this._actors.length;while(a--){this._actors[a].setLookupTable(c,this.scalarMIN,this.scalarMAX)}};nucleo.Scene.prototype.reset=function(){var a=this._actors.length;while(a--){this._actors[a]=null}this._actors=[];nucleo.c.actor=null;this.computeBoundingBox()};nucleo.Scene.prototype.getActorByName=function(a){a=a.replace(/\.[^/.]+$/,"");
var c=this._actors.length;while(c--){if(this._actors[c].name==a){return this._actors[c]}}return null};nucleo.Scene.prototype.getActorByUID=function(a){var c=this._actors.length;while(c--){if(this._actors[c].UID==a){return this._actors[c]}}return null};nucleo.Scene.prototype.getActor=function(c){var a=this.getActorByName(c);if(a==null){a=this.getActorByUID(c)}return a};nucleo.Scene.prototype.getActorsThat=function(f){var a=[];var e=this._actors.length;while(e--){if(f(this._actors[e])){a.push(e)}}var d=[];
var c=a.length;while(c--){d.push(this._actors[a[c]])}return d};nucleo.Scene.prototype.setOpacity=function(e,a){if(a==null){var c=this._actors.length;while(c--){this._actors[c].setOpacity(e)}}else{var d=this.getActorByName(a);d.setOpacity(e)}};nucleo.Scene.prototype.flipNormals=function(){var a=this._actors.length;while(a--){this._actors[a].flipNormals()}};nucleo.Scene.prototype.setVisualizationMode=function(c){if(c==null||c==undefined){return}var a=this._actors.length;while(a--){this._actors[a].setVisualizationMode(c)
}};nucleo.Scene.prototype.setAnimation=function(c){if(c instanceof nucleo.FrameAnimation){this.frameAnimation=c;this.frameAnimation.scene=this;var a=this.views.length;while(a--){this.views[a].renderer.setMode(nucleo.Renderer.MODE.ANIMFRAME)}nucleo.util.console("Scene: animation added")}};nucleo.Scene.prototype.clearAnimation=function(){if(this.frameAnimation){this.frameAnimation.scene=null;this.frameAnimation=null}};nucleo.Scene.prototype.getActorNames=function(){var c=[];var a=this._actors.length;
while(a--){c.push(this._actors[a].name)}return c};nucleo.Scene.prototype.getPickableActors=function(){function a(c){return c.isPickable()}return this.getActorsThat(a)};nucleo.Scene.prototype.getActorByCellUID=function(a){var e=[];var c=this._actors.length;while(c--){var d=this._actors[c];if(d.mesh!=undefined&&d.mesh.hasCell(a)){return d}}return null};nucleo.Scene.prototype.createActorGroup=function(a,d){var e=undefined;if(this.getActorGroup(a)!=undefined){alert("Scene.createActorGroup: an actor group with the name "+a+" already exists");
throw ("Scene.createActorGroup: an actor group with the name "+a+" already exists")}try{e=new ActorGroup(this,a,d);this._groups.push(e)}catch(c){if(c instanceof vxlActorGroupException){alert(c.messages)}}finally{return e}};nucleo.Scene.prototype.getActorGroup=function(a){var c=this._groups.length;while(c--){if(this._groups[c].name==a){return this._groups[c]}}return undefined};nucleo.SceneToys=function SceneToys(a){this.scene=a;this.list=[];this.axis=new nucleo.Axis();this.boundingbox=new nucleo.BoundingBox();
this.floor=new nucleo.Floor();this.list.push(this.axis);this.list.push(this.boundingbox);this.list.push(this.floor)};nucleo.SceneToys.prototype.update=function(){this.axis.setCentre(this.scene.centre);this.boundingbox.setBoundingBox(this.scene.bb)};nucleo.LookupTable=function LookupTable(){this.ID=null;this.map=null;this.max=Number.MIN_VALUE;this.min=Number.MAX_VALUE};nucleo.LookupTable.prototype.load=function(a,d){this.ID=a;this.map=d;for(var c in this.map){var e=Number(c);if(e<this.min){this.min=e
}else{if(e>=this.max){this.max=e}}}};nucleo.LookupTable.prototype._scale=function(d,c,a){return d*this.max/a};nucleo.LookupTable.prototype.getColor=function(j,f,a){var h=this._scale(j,f,a);var d=this;var e=Math.round(h);if(e>=d.min&&e<=d.max){var k=[d.map[e][0],d.map[e][1],d.map[e][2]];return k}else{if(e<d.min){return[d.map[d.min][0],d.map[d.min][1],d.map[d.min][2]]}else{if(e>d.max){return[d.map[d.max][0],d.map[d.max][1],d.map[d.max][2]]}else{alert("assertion error in getColor routine");return[d.map[d.min][0],d.map[d.min][1],d.map[d.min][2]]
}}}};nucleo.LookupTable.prototype.getColors=function(f,e,a){var j=[];for(var d=0;d<f.length;d++){var h=this.getColor(f[d],e,a);j.push(h[0]);j.push(h[1]);j.push(h[2])}return j};nucleo.LookupTableManager=function LookupTableManager(){this._hashmap={};this._location="";nucleo.Notifier.instance.publish(nucleo.EVENTS.DEFAULT_LUT_LOADED,this)};nucleo.LookupTableManager.prototype.setLocation=function(a){this._location=a};nucleo.LookupTableManager.prototype.load=function(d){if(this.isLoaded(d)){return}var e=this;
var h=this._location+"/"+d+".lut";var j=h+"?nocache="+(new Date()).getTime();var c=function(l,k){return function(m,n){l._handle(k,m)}};var a=function(k){return function(n,l,m){if(m.code=1012){alert("The file "+k+" could not be accessed. \n\nPlease make sure that the path is correct and that you have the right pemissions")}else{alert("There was a problem loading the file "+k+". HTTP error code:"+n.status)}}};var f=$.ajax({url:j,type:"GET",dataType:"json",mimeType:"text/plain",success:c(e,d),error:a(h)})
};nucleo.LookupTableManager.prototype._handle=function(a,d){var c=new nucleo.LookupTable();c.load(a,d);this._hashmap[a]=c;if(c.ID==nucleo.def.lut.main){nucleo.Notifier.instance.fire(nucleo.EVENTS.DEFAULT_LUT_LOADED,this)}};nucleo.LookupTableManager.prototype.isLoaded=function(a){return this._hashmap[a]!=undefined};nucleo.LookupTableManager.prototype.get=function(a){return this._hashmap[a]};nucleo.LookupTableManager.prototype.getAllLoaded=function(){var a=[];for(lut in this._hashmap){a.push(this._hashmap[lut].ID)
}return a};nucleo.LookupTableManager.prototype.allLoaded=function(){var c=0;for(var a in this._hashmap){c++}return(nucleo.define.LookupTable.list.length==c)};nucleo.LookupTableManager.prototype.loadAll=function(){for(var a=0;a<nucleo.define.LookupTable.list.length;a++){this.load(nucleo.define.LookupTable.list[a])}};nucleo.LookupTableManager.instance=new nucleo.LookupTableManager();nucleo.BoundingBox=function BoundingBox(){var a=new nucleo.Model();a.load("bounding box",nucleo.define.Model.BoundingBox);
nucleo.Actor.call(this,a);this.bb=this.setBoundingBox([-1,-1,-1,1,1,1]);this.mode=nucleo.Actor.MODE.WIREFRAME;this.visible=false;this.toy=true};nucleo.BoundingBox.prototype=new nucleo.Actor();nucleo.BoundingBox.prototype.constructor=nucleo.BoundingBox;nucleo.BoundingBox.prototype.setBoundingBox=function(a){this.bb=[a[0],a[1],a[2],a[0],a[4],a[2],a[3],a[4],a[2],a[3],a[1],a[2],a[0],a[1],a[5],a[0],a[4],a[5],a[3],a[4],a[5],a[3],a[1],a[5]];this.model.vertices=this.bb;this.reallocate()};nucleo.Axis=function Axis(){var a=new nucleo.Model();
a.load("axis",nucleo.define.Model.Axis);nucleo.Actor.call(this,a);this.centre=[0,0,0];this.mode=nucleo.Actor.MODE.WIREFRAME;this.visible=false;this.toy=true};nucleo.Axis.prototype=new nucleo.Actor();nucleo.Axis.prototype.constructor=nucleo.Axis;nucleo.Axis.prototype.setCentre=function(d){var c=d[0];var f=d[1];var e=d[2];this.centre[0]=c;this.centre[1]=f;this.centre[2]=e;var a=this.model.vertices;a[0]=c-1;a[1]=f;a[2]=e;a[3]=c+1;a[4]=f;a[5]=e;a[6]=c;a[7]=f-2;a[8]=e;a[9]=c;a[10]=f+2;a[11]=e;a[12]=c;
a[13]=f;a[14]=e-1;a[15]=c;a[16]=f;a[17]=e+1};nucleo.Floor=function Floor(){var a=new nucleo.Model();a.load("floor",nucleo.define.Model.Floor);nucleo.Actor.call(this,a);this.mode=nucleo.Actor.MODE.WIREFRAME;this.visible=false;this.toy=true};nucleo.Floor.prototype=new nucleo.Actor();nucleo.Floor.prototype.constructor=nucleo.Floor;nucleo.Floor.prototype.setGrid=function(j,k){var h=j;var c=2*h/k;var f=2*h/c;var d=[];var e=[];for(var a=0;a<=c;a++){d[6*a]=-h;d[6*a+1]=0;d[6*a+2]=-h+(a*f);d[6*a+3]=h;d[6*a+4]=0;
d[6*a+5]=-h+(a*f);d[6*(c+1)+6*a]=-h+(a*f);d[6*(c+1)+6*a+1]=0;d[6*(c+1)+6*a+2]=-h;d[6*(c+1)+6*a+3]=-h+(a*f);d[6*(c+1)+6*a+4]=0;d[6*(c+1)+6*a+5]=h;e[2*a]=2*a;e[2*a+1]=2*a+1;e[2*(c+1)+2*a]=2*(c+1)+2*a;e[2*(c+1)+2*a+1]=2*(c+1)+2*a+1}this.model.vertices=d.slice(0);this.model.wireframe=e.slice(0);this.visible=true;this.reallocate()};nucleo.View=function View(f,d,h){this.UID=nucleo.util.generateUID();this.canvas=document.getElementById(f);if(this.canvas==null||this.canvas==undefined){alert("View: the canvas "+f+" does not exist.");
throw ("View: the canvas "+f+" does not exist.")}this.name=f;this.width=this.canvas.width;this.height=this.canvas.height;this.clearDepth=1;this.backgroundColor=nucleo.View.BACKGROUND.slice(0);this._dragndrop=false;this.renderer=new nucleo.Renderer(this);this.cameraman=new nucleo.CameraManager(this);this.interactor=new nucleo.TrackerInteractor();this._fullscreenFlag=true;if(d!=null&&d instanceof nucleo.Scene){this.scene=d}else{this.scene=new nucleo.Scene()}this.scene.views.push(this);if(h==undefined){h=true
...
a++){c.push(e+d[a])}}else{throw Exception("not a valid argument")}}if(h!==undefined&&h!==null){this.scene.setLoadingMode(h)}nucleo.ModelManager.instance.loadList(c,this.scene)};nucleo.VTKReader=function VTKReader(a){this.scene=a;nucleo.Notifier.instance.publish(nucleo.EVENTS.READER_DONE,this)};nucleo.VTKReader.prototype.isSupported=function(){return(window.File&&window.FileReader&&window.FileList&&window.Blob)};nucleo.VTKReader.prototype.readFile=function(e){var a=this;var d=e.name;if(d.length-4!==d.indexOf(".vtk")){throw"VTKReader.read: the file "+e+" is not a VTK file"
}modelname=d.replace(/\.[^/.]+$/,"");var c=new FileReader();c.onload=function(j){document.body.style.cursor="default";var h=j.target.result.trim();var f=h.split(/\r\n|\r|\n/);a._parse(modelname,f);nucleo.Notifier.instance.fire(nucleo.EVENTS.READER_DONE,a)};document.body.style.cursor="wait";c.readAsText(e)};nucleo.VTKReader.prototype.readText=function(c,d){document.body.style.cursor="wait";var a=d.split(/\r\n|\r|\n/);this._parse(c,a);nucleo.Notifier.instance.fire(nucleo.EVENTS.READER_DONE,this);document.body.style.cursor="default"
};nucleo.VTKReader.prototype._parse=function(C,d){var f=65536*3;var z="";var e=0;var c="NOWHERE";var j=0;var m=[];var l=[];var x=[];var h=[];var q=[];var u="SOLID";var w={NOWHERE:0,POINTS:1,LINES:2,POLYGONS:3,POINT_DATA:4,NORMALS:5,CELL_DATA:6,TEXTURE_COORDINATES:7,SCALARS:8,LOOKUP_TABLE:9,COLOR_SCALARS:10};this.json={name:C};function o(n){var E=n.length-1;if(E!=n[0]){throw"Assertion Error. Inconsistent line"}var v=n.splice(1,E);for(var D=0;D<E-1;D+=1){l.push(parseInt(v[D]));l.push(parseInt(v[D+1]))
}}for(var j=0;j<d.length;j++){try{if(d[j].indexOf("POINTS")==0){console.log(d[j]);c=w.POINTS;continue}else{if(d[j].indexOf("LINES")==0){console.log(d[j]);c=w.LINES;u="LINES";continue}else{if(d[j].indexOf("TRIANGLE_STRIPS")==0){console.log(d[j]);alert("vxlVTKParser ERROR: \nTriangle Strips Not Supported. Please triangulate first");throw ("Triangle Strips Not Supported. Please triangulate first")}else{if(d[j].indexOf("POLYGONS")==0){console.log(d[j]);c=w.POLYGONS;continue}else{if(d[j].indexOf("POINT_DATA")==0){c=w.POINT_DATA;
continue}else{if(d[j].indexOf("NORMALS")==0){console.log(d[j]);c=w.NORMALS;continue}else{if(d[j].indexOf("CELL_DATA")==0){console.log(d[j]);c=w.CELL_DATA;continue}else{if(d[j].indexOf("TEXTURE_COORDINATES")==0){console.log(d[j]);c=w.TEXTURE_COORDINATES;continue}else{if(d[j].indexOf("SCALARS")==0){console.log(d[j]);c=w.SCALARS;continue}else{if(d[j].indexOf("LOOKUP_TABLE")==0){console.log(d[j]);c=w.LOOKUP_TABLE;continue}else{if(d[j].indexOf("COLOR_SCALARS")==0){console.log(d[j]);c=w.COLOR_SCALARS;continue
}else{if(c==w.POINTS){var s=d[j].trim().split(" ");if(s==""){continue}for(var A=0;A<s.length;A++){m.push(parseFloat(s[A]))}}else{if(c==w.LINES){var t=d[j].trim().split(" ");if(t==""){continue}o(t)}else{if(c==w.POLYGONS){var a=d[j].trim().split(" ");if(a==""){continue}if(a.length>0&&a[0]!="3"){throw"Error: please make sure your vtk file contains triangles instead of polygons (triangulate first)"}for(var A=1;A<a.length;A++){l.push(parseInt(a[A]))}}else{if(c==w.LOOKUP_TABLE){if(d[j].indexOf("LOOKUP_TABLE")==0){continue
}else{var B=d[j].trim().split(" ");if(B==""){continue}for(var A=0;A<B.length;A++){h.push(parseFloat(B[A]))}}}else{if(c==w.COLOR_SCALARS){var y=d[j].trim().split(" ");if(y==""){continue}for(var A=0;A<y.length;A++){q.push(parseFloat(y[A]))}}else{if(c==w.NORMALS){var y=d[j].trim().split(" ");if(y==""){continue}for(var A=0;A<y.length;A++){x.push(parseFloat(y[A]))}}}}}}}}}}}}}}}}}}}catch(k){console.log("Error while processing line "+j.toString());throw k}}this.json.vertices=m;this.json.mode=u;if(l.length>0){this.json.indices=l
}if(x.length>0){this.json.normals=x}if(q.length>0){this.json.colors=q}if(h.length>0){this.json.scalars=h}};nucleo.VTKReader.prototype.getModel=function(){var a=new Model(this.json.name,this.json);this.json=null;delete this.json;return a};nucleo.ModelManager=function ModelManager(){this.toLoad=[];this.models=[];this.readers={};var a=nucleo.EVENTS;nucleo.Notifier.instance.publish([a.MODELS_LOADING,a.MODEL_NEW,a.MODELS_LOADED],this);nucleo.Notifier.instance.subscribe(a.READER_DONE,this);this.registerReader("vtk",nucleo.VTKReader)
};nucleo.ModelManager.prototype.handleEvent=function(e,c){var a=c;var d=a.getModel();this.add(d,a.scene)};nucleo.ModelManager.prototype.registerReader=function(c,a){this.readers[c]=new a()};nucleo.ModelManager.prototype.load=function(l,e){var j,h,n,a,c,d=this,k=l.replace(/^.*[\\\/]/,""),f=k.split(".")[0],m=k.split(".")[1];if(d.isModelLoaded(f)){return}if(m==="vtk"){n="text";a="text/plain"}else{if(m==="json"){n="json";a="application/json"}else{throw ("ModelManager.load ERROR: Unknown filetype ["+m+"]")
}}c=encodeURI(l+"?nocache="+(new Date()).getTime());nucleo.util.console("ModelManager.load: Requesting "+l+"...");nucleo.Notifier.instance.fire(nucleo.EVENTS.MODELS_LOADING,this);j=function(q,o,s){switch(m){case"json":return function(t){t.uri=k;t.path=nucleo.util.getPath(l);t.name=o;q._createActorInScene(t,s)};break;case"vtk":return function(u){var t=new nucleo.VTKReader(s);t.readText(o,u)}}};h=function(o){return function(t,q,s){if(q=1012){alert("The file "+o+" could not be accessed. \n\nPlease make sure that the path is correct and that you have the right pemissions")
}else{alert("There was a problem loading the file "+o+". HTTP error code:"+q+" "+s)}}};nucleo.ajax({url:c,type:"GET",dataType:n,mimeType:a,success:j(d,f,e),error:h(l)})};nucleo.ModelManager.prototype.loadList=function(d,a){this.toLoad=d.slice(0);nucleo.util.console("ModelManager.loadList: models to load ->"+this.toLoad.length);for(var c=0;c<this.toLoad.length;c++){this.load(this.toLoad[c],a)}};nucleo.ModelManager.prototype._createActorInScene=function(f,a){var d=this,e;if(f instanceof nucleo.Model){e=f
}else{e=new nucleo.Model(f.name,f)}function c(){e.loaded=true;d.models.push(e);d.toLoad.splice(name,1);nucleo.util.console("ModelManager: model "+e.name+" created.");if(a!=undefined&&a instanceof nucleo.Scene){nucleo.util.console("ModelManager: notifying the scene...");if(a.loadingMode==nucleo.Model.LOADING_MODE.LIVE){var j=new nucleo.Actor(e);a.addActor(j)}else{if(a.loadingMode==nucleo.Model.LOADING_MODE.LATER){if(d.allLoaded()){var h=models.length;while(h--){this.createActor(models[h])}}}}}else{if(a.loadingMode==nucleo.Model.LOADING_MODE.DETACHED){}}}if(d.allLoaded()){nucleo.Notifier.instance.fire(nucleo.EVENTS.MODELS_LOADED,d)
}else{nucleo.Notifier.instance.fire(nucleo.EVENTS.MODEL_NEW,d)}setTimeout(function(){c()},0)};nucleo.ModelManager.prototype.reset=function(){this.firstLoadedModel=false;for(var a=0;a<this.models.length;a++){this.models[a]=null}this.models=[];this.toLoad=[]};nucleo.ModelManager.prototype.isModelLoaded=function(a){for(var c=0;c<this.models.length;c++){if(this.models[c].name==a){return true}}return false};nucleo.ModelManager.prototype.allLoaded=function(){return(this.toLoad.length==0)};nucleo.ModelManager.prototype.getModelByName=function(c){for(var d=0,a=this.models.length;
d<a;d+=1){if(this.models[d].name==c){return this.models[d]}}return null};nucleo.ModelManager.instance=new nucleo.ModelManager();nucleo.FrameAnimation=function FrameAnimation(a){this.scene=null;this.actorByFrameMap=[];this.activeFrame=1;this.mark=1;this._running=false;this.frameCount=0;this.renderRate=500;this._startDate=undefined;this._time=0;this._setup(a)};nucleo.FrameAnimation.prototype.addActorToFrame=function(c,a){if(typeof(this.actorByFrameMap[c])=="undefined"){this.actorByFrameMap[c]=new Array()
}if(this.actorByFrameMap[c].indexOf(a)==-1){this.actorByFrameMap[c].push(a)}if(c>this.frameCount){this.frameCount=c}};nucleo.FrameAnimation.prototype._setup=function(h){this.activeFrame=1;for(var e in h){var c=h[e];var j=parseInt(e.substr(5,e.length));for(var d=0,a=c.length;d<a;d+=1){this.addActorToFrame(j,c[d])}}nucleo.util.console("FrameAnimation: Setup finished.")};nucleo.FrameAnimation.prototype.start=function(a){if(this.scene==null){throw"FrameAnimation: the animation is not associated with any scene. Please use scene.setFrameAnimation method"
}this._startDate=new Date().getTime();this._time=0;this._running=true;if(a!=undefined&&a>=0){this.renderRate=a}this._timeUp()};nucleo.FrameAnimation.prototype._timeUp=function(){if(!this._running){return}this.nextFrame();if(this._time==this.renderRate*100){this._time=0;this._startDate=new Date().getTime()}this._time+=this.renderRate;var a=(new Date().getTime()-this._startDate)-this._time;if(a>this.renderRate){a=0}setTimeout((function(c){return function(){c._timeUp()}})(this),this.renderRate-a)};nucleo.FrameAnimation.prototype.stop=function(){this._running=false
};nucleo.FrameAnimation.prototype.setFrameRate=function(a){if(a<=0){return}this.stop();this.renderRate=a;this.start()};nucleo.FrameAnimation.prototype.update=function(){if(this.scene==null){throw"FrameAnimation: the animation is not associated with any scene. Please use scene.setFrameAnimation method"}var c=this.getPreviousFrames(1);var a=c.length;for(var e=0;e<a;e++){var f=this.scene.getActorByName(c[e]);if(f!=null){f.setVisible(false)}}var d=this.actorByFrameMap[this.activeFrame];a=d.length;for(e=0;
e<a;e++){f=this.scene.getActorByName(d[e]);if(f!=null){f.setVisible(true)}}};nucleo.FrameAnimation.prototype.isValidFrame=function(a){return(a>=1&&a<=this.frameCount)};nucleo.FrameAnimation.prototype.nextFrame=function(){if(this.activeFrame<this.frameCount){this.activeFrame++}else{this.activeFrame=1}};nucleo.FrameAnimation.prototype.getNextFrames=function(d){var f=[];var h=this.activeFrame;if(d>this.frameCount){d=this.frameCount}for(var a=1;a<=d;a++){var e=h+a;if(this.isValidFrame(e)){f.push(e)}else{f.push(e-this.frameCount)
}}nucleo.util.console("Animation: next frames: "+f);return f};nucleo.FrameAnimation.prototype.getPreviousFrames=function(d){var f=[];var h=this.activeFrame;if(d>this.frameCount){d=this.frameCount}for(var a=1;a<=d;a++){var e=h-a;if(this.isValidFrame(e)){f.push(e)}else{f.push(this.frameCount+e)}}nucleo.util.console("Animation: previous frames: "+f);return f};nucleo.FrameAnimation.prototype.setFrame=function(a){if(a>=1&&a<=this.frameCount){this.activeFrame=a;this.render()}};nucleo.Texture=function Texture(c){var a=this;
this.image=new Image();this.image.onload=function(){a._onLoad()};this.image.onError=function(){a._onError()};this.uri=c;if(this.uri!=undefined){this.load(this.uri)}this.mag=nucleo.Texture.FILTER.LINEAR;this.min=nucleo.Texture.FILTER.LINEAR_MIPMAP_LINEAR;this.loaded=false};nucleo.Texture.FILTER=nucleo.define.Texture.FILTER;nucleo.Texture.prototype.setMagFilter=function(a){this.mag=a};nucleo.Texture.prototype.setMinFilter=function(a){this.min=a};nucleo.Texture.prototype.load=function(a){this.image.src=a
};nucleo.Texture.prototype._onError=function(){console.info("Texture: the texture "+this.uri+" could not be found.");this.loaded=false};nucleo.Texture.prototype._onLoad=function(){this.loaded=true};nucleo.Texture.prototype.getMagFilter=function(c){var a=nucleo.def.texture.filter;switch(this.mag){case a.LINEAR:return c.LINEAR;break;case a.NEAREST:return c.NEAREST;break;default:return c.NEAREST}};nucleo.Texture.prototype.getMinFilter=function(c){var a=nucleo.Texture.FILTER;switch(this.min){case a.LINEAR:return c.LINEAR;
break;case a.NEAREST:return c.NEAREST;break;case a.LINEAR_MIPMAP_LINEAR:return c.LINEAR_MIPMAP_LINEAR;break;case a.LINEAR_MIPMAP_NEAREST:return c.LINEAR_MIPMAP_NEAREST;break;case a.NEAREST_MIPMAP_LINEAR:return c.NEAREST_MIPMAP_LINEAR;break;case a.NEAREST_MIPMAP_NEAREST:return c.NEAREST_MIPMAP_NEAREST;break;default:return c.NEAREST}};nucleo.Material=function Material(a){this.ambient=nucleo.Material.AMBIENT;this.diffuse=nucleo.Material.DIFFUSE;this.specular=nucleo.Material.SPECULAR;this.shininess=nucleo.Material.SHININESS;
this.opacity=nucleo.Material.OPACITY;this.shading=nucleo.Material.SHADING;this.texture=undefined;this.colors=undefined;if(a){this.getFrom(a)}};nucleo.Material.AMBIENT=nucleo.define.Material.AMBIENT;nucleo.Material.DIFFUSE=nucleo.define.Material.DIFFUSE;nucleo.Material.SPECULAR=nucleo.define.Material.SPECULAR;nucleo.Material.SHININESS=nucleo.define.Material.SHININESS;nucleo.Material.OPACITY=nucleo.define.Material.OPACITY;nucleo.Material.SHADING=nucleo.define.Material.SHADING;nucleo.Material.prototype.getFrom=function(a){if(a.ambient!=undefined){this.ambient=a.ambient.slice(0)
}if(a.diffuse!=undefined){this.diffuse=a.diffuse.slice(0)}if(a.specular!=undefined){this.specular=a.specular.slice(0)}if(a.color!=undefined){this.diffuse=a.color.slice(0)}if(a.shininess!=undefined){this.shininess=a.shininess}if(a.opacity!=undefined){this.opacity=a.opacity}if(a.shading!=undefined){this.shading=a.shading}if(a.texture!=undefined){this.texture=new Texture(a.path+a.texture)}if(a.colors!=undefined){this.colors=a.colors}return this};nucleo.Material.prototype.clone=function(){var d=new Material();
var a=this;for(var c in a){if(a.hasOwnProperty(c)){if(a[c] instanceof Array){d[c]=a[c].slice(0)}else{d[c]=a[c]}}}return d};nucleo.Renderable=function Renderable(a){if(a==undefined){throw ("Renderable: the actor can not be undefined")}this.actor=a;this.parts=[];this.update(nucleo.Renderable.TASK.CREATE)};nucleo.Renderable.prototype.update=function(a){if(a==undefined){throw ("Renderable.update: Please specify a task")}var c=this._getModel();if(c==undefined){return}switch(c.type){case nucleo.Model.TYPE.MESH:this._processMesh(c,a);
break;case nucleo.Model.TYPE.BIG_DATA:this._processBigData(c,a);break}};nucleo.Renderable.prototype._getModel=function(){var a=this.actor;if(a.mesh&&a.mesh.model){return a.mesh.model}else{if(a.model.type==nucleo.Model.TYPE.BIG_DATA){return a.model}else{return undefined}}};nucleo.Renderable.prototype._processMesh=function(k,d){var n=Model.MAX_NUM_INDICES;var l=Math.floor(k.indices.length/n),f=k.indices.length%n;if(d==Renderable.TASK.CREATE){this.parts=[];for(var h=0;h<=l;h+=1){var c=new Model(k.name+"-renderable-part-"+h);
var m=h*n;var j=m+n;var a=h*n*3;var e=a+n*3;if(h==l){m=l*n;j=m+f;a=l*n*3;e=a+f*3;if(f==0){break}}c.indices=this._reindex(k.indices.slice(m,j));c.vertices=k.vertices.slice(a,e);if(k.normals&&k.normals.length>0){c.normals=k.normals.slice(a,e)}if(k.colors&&k.colors.length>0){c.colors=k.colors.slice(a,e)}if(k.pickingColors){c.pickingColors=k.pickingColors.slice(a,e)}c.update();this.parts.push(c)}}else{if(d==Renderable.TASK.UPDATE_COLORS){for(var h=0;h<=l;h+=1){var c=this.parts[h];var a=h*n*3;var e=a+n*3;
if(h==l){a=l*n*3;e=a+f*3;if(f==0){break}}if(k.colors&&k.colors.length>0){c.colors=k.colors.slice(a,e)}}}}};nucleo.Renderable.prototype._reindex=function(d){var c=d.min();var a=d.length;while(a--){d[a]=d[a]-c}return d};nucleo.Renderable.prototype._processBigData=function(h,n){var o=h.indices,v=Model.MAX_NUM_INDICES;if(h.mode==nucleo.Actor.MODE.LINES){v=v-1}var q=o.length,f=o.max(),m=this.actor.material,k=(m.colors&&m.colors.length>0),d=(h.normals&&h.normals.length>0),u=(h.scalars&&h.scalars.length>0),j=h.mode;
data={vertices:[],indices:[],mode:j};if(k){data.colors=[]}if(d){data.normals=[]}if(u){data.scalars=[]}var s={},t=1,l=0,a=0;for(var e=0;e<q;e+=1){index=o[e];if(s[index]==undefined){s[index]=l;vertex=this._getVertexData(index);data.vertices.push.apply(data.vertices,vertex.coords);if(d){data.normals.push.apply(data.normals,vertex.normal)}if(k){data.colors.push.apply(data.colors,vertex.color)}if(u){data.scalars.push(vertex.scalar)}l+=1}data.indices.push(s[index]);if((l==v+1)||(e==q-1)){var c=new Model(h.name+"-renderable-part-"+t,data);
c.update();nucleo.util.console("Creating part "+c.name+" ["+t+"]",true);this.parts.push(c);l=0;t+=1;c={vertices:[],indices:[],mode:j};s={};data={vertices:[],indices:[],mode:j};if(k){data.colors=[]}if(d){data.normals=[]}if(u){data.scalars=[]}}}};nucleo.Renderable.prototype._getVertexData=function(c){var d=this.actor.material;var a=this.actor.model;var e={};e.coords=a.vertices.slice(c*3,c*3+3);if(a.normals){e.normal=a.normals.slice(c*3,c*3+3)}if(d.colors){e.color=d.colors.slice(c*3,c*3+3)}if(a.scalars){e.scalar=a.scalars[c]
}return e};nucleo.ActorGroup=function ActorGroup(d,a,c){this.scene=d;this.name=a;this.list=[];this.addList(c)};nucleo.ActorGroup.prototype.addList=function(f){var e=[];for(var c=0,h=f.length;c<h;c+=1){var d=f[c];if(d instanceof nucleo.Actor&&d.scene==this.scene){this.list.push(d)}else{if(typeof(d)=="string"){var a=this.scene.getActor(d);if(a!=null){this.list.push(a)}else{e.push("Could not find actor: "+d)}}else{e.push("The object "+d+" is not of the expected type (Actor, string)")}}}if(e.length!=0){this.list=[];
throw new vxlActorGroupException(e)}};nucleo.ActorGroup.prototype.add=function(c){var d=[];if(c instanceof nucleo.Actor&&c.scene==this.scene){this.list.push(c)}else{if(typeof(c)=="string"){var a=this.scene.getActor(c);if(a!=null){this.list.push(a)}else{d.push("Could not find actor: "+c)}}else{d.push("The object "+c+" is not of the expected type (Actor, string)")}}if(d.length!=0){this.list=[];throw new vxlActorGroupException(d)}};nucleo.ActorGroup.prototype.hasActor=function(c){if(c instanceof nucleo.Actor){return(this.list.indexOf(c)!=-1)
}else{if(typeof(c)=="string"){var a=this.scene.getActor(c);return(a!=null)}else{return false}}};nucleo.ActorGroup.prototype.remove=function(c){var a=-1;if(c instanceof nucleo.Actor){a=this.list.indexOf(c)}else{if(typeof(c)=="string"){actorObject=this.scene.getActor(c);a=this.list.indexOf(actorObject)}}if(a!=1){this.list.splice(a,1)}else{throw new vxlActorGroupException(["the actor "+c+" was not found in the group "+this.name])}};nucleo.ActorGroup.prototype.reset=function(a){this.list=[];if(a!=undefined&&a instanceof Array){this.addList(a)
}};nucleo.ActorGroup.prototype.size=function(){return this.list.length};nucleo.ActorGroup.prototype.setProperty=function(d,c){for(var a=0,e=this.list.length;a<e;a+=1){this.list[a].setProperty(d,c,nucleo.def.actor)}return this};nucleo.ActorGroup.prototype._apply=function(a,d){for(var c=0,e=this.list.length;c<e;c+=1){a.apply(this.list[c],d)}return this};nucleo.ActorGroup.prototype.flipNormals=function(){return this._apply(Actor.prototype.flipNormals)};nucleo.ActorGroup.prototype.rotateX=function(a){return this._apply(Actor.prototype.rotateX,[a])
};nucleo.ActorGroup.prototype.rotateY=function(a){return this._apply(Actor.prototype.rotateY,[a])};nucleo.ActorGroup.prototype.rotateZ=function(a){return this._apply(Actor.prototype.rotateZ,[a])};nucleo.ActorGroup.prototype.translate=function(a,d,c){return this._apply(Actor.prototype.translate,[a,d,c])};nucleo.ActorGroup.prototype.setColor=function(a){return this.setProperty("color",a)};nucleo.ActorGroup.prototype.setLookupTable=function(d,c,a){return this._apply(Actor.prototype.setLookupTable,[d,c,a])
};nucleo.ActorGroup.prototype.setOpacity=function(a){return this.setProperty("opacity",a)};nucleo.ActorGroup.prototype.setPicker=function(a,c){return this._apply(Actor.prototype.setPicker,[a,c])};nucleo.ActorGroup.prototype.setPosition=function(a,d,c){return this._apply(Actor.prototype.setPosition,[a,d,c])};nucleo.ActorGroup.prototype.setScale=function(a,d,c){return this._apply(Actor.prototype.setScale,[a,d,c])};nucleo.ActorGroup.prototype.setShininess=function(a){return this.setProperty("shininess",a)
};nucleo.ActorGroup.prototype.setVisible=function(a){return this.setProperty("visible",a)};nucleo.ActorGroup.prototype.setShading=function(a){return this.setProperty("shading",a)};nucleo.ActorGroup.prototype.setVisualizationMode=function(a){return this._apply(Actor.prototype.setVisualizationMode,[a])};function vxlActorGroupException(a){this.messages=a}nucleo.Silo=function Silo(a){this._mode=a;this._actors=[];this.data={};this.cached={};this.added={};this.prepare()};nucleo.Silo.prototype.populate=function(d){var e=d.length;
for(var a=0;a<e;a+=1){var c=d[a];if(this.cached[c.UID]==undefined&&c.mode==this._mode){this._actors.push(c);this.cached[c.UID]=c}}};nucleo.Silo.prototype.prepare=function(){var a={mixin:[],index:[],matrix:[[],[],[],[]]};this.data=a};nucleo.Silo.prototype.process=function(){var d=this._actors.length;for(var a=0;a<d;a+=1){var c=this._actors[a];if(this.added[c.UID]==undefined){this.processActor(this._actors[a]);this.added[c.UID]=c}}};nucleo.Silo.prototype.processActor=function(f){var j=f.model;var m=f.material;
var o=this.data.mixin;var l=this.data.index;var n=this.data.matrix;var a=j.vertices.length;var e=j.indices.length;var k=Array.prototype.slice.call(f._matrix);if(m.colors!=undefined){}else{for(var h=0;h<a;h+=3){o=o.concat([j.vertices[h],j.vertices[h+1],j.vertices[h+2],j.normals[h],j.normals[h+1],j.normals[h+2],m.diffuse[0],m.diffuse[1],m.diffuse[2]]);n[0]=n[0].concat(k.slice(0,4));n[1]=n[1].concat(k.slice(4,8));n[2]=n[2].concat(k.slice(8,12));n[3]=n[3].concat(k.slice(12,16))}}var c=j.indices.slice(0);
if(l.length>0){var d=l.max()+1;for(var h=0;h<e;h+=1){c[h]+=d}l=l.concat(c)}else{l=c}this.data.mixin=o;this.data.index=l;this.data.matrix=n};nucleo.DashEngine=function DashEngine(a){nucleo.Engine.call(this);this.silos={};this._createSilos();this._gl_buffers={};this._createGLBuffers();this.essl=nucleo.util.extend(nucleo.define.ESSL,{ACTOR_MATRIX:"aActorMatrix"})};nucleo.DashEngine.prototype=new nucleo.Engine();nucleo.DashEngine.prototype.constructor=nucleo.DashEngine;nucleo.DashEngine.prototype.configure=function(){nucleo.Engine.prototype.configure.call(this);
var a=this.gl;a.clearDepth(1);a.disable(a.CULL_FACE)};nucleo.DashEngine.prototype._createSilos=function(){for(key in nucleo.Actor.MODE){this.silos[key]={};this.silos[key]=new Silo(key)}};nucleo.DashEngine.prototype._createGLBuffers=function(){var a=this.renderer.gl;for(key in nucleo.Actor.MODE){this._gl_buffers[key]={};this._gl_buffers[key].mixin=a.createBuffer();this._gl_buffers[key].index=a.createBuffer();this._gl_buffers[key].matrix=a.createBuffer()}};nucleo.DashEngine.prototype.allocate=function(c){var a=c._actors;
for(key in this.silos){this.silos[key].populate(a);this.silos[key].process()}};nucleo.DashEngine.prototype.render=function(f){var d=this.renderer;var c=d.transforms;var a=d.pm;var h=d.gl;var e=this.essl;if(f._actors.length==0){return}c.update();a.setUniform(e.PERSPECTIVE_MATRIX,c.pMatrix);a.setUniform(e.MODEL_VIEW_MATRIX,c.mvMatrix);a.setUniform(e.NORMAL_MATRIX,c.nMatrix);a.setUniform(e.MVP_MATRIX,c.mvpMatrix);this._renderSolid()};nucleo.DashEngine.prototype.deallocate=function(a){};nucleo.DashEngine.prototype.enableMatrixAttribute=function(c){var a=this.renderer;
var l=a.gl;var d=a.pm._essl;var e=nucleo.Actor.MODE;var j=this.silos[e.SOLID];var k=j.data;var o=l.getAttribLocation(d,c);l.enableVertexAttribArray(o);l.enableVertexAttribArray(o+1);l.enableVertexAttribArray(o+2);l.enableVertexAttribArray(o+3);var q=l.createBuffer();var n=l.createBuffer();var h=l.createBuffer();var f=l.createBuffer();l.bindBuffer(l.ARRAY_BUFFER,q);l.bufferData(l.ARRAY_BUFFER,new Float32Array(k.matrix[0]),l.STATIC_DRAW);l.vertexAttribPointer(o,4,l.FLOAT,false,0,0);l.bindBuffer(l.ARRAY_BUFFER,n);
l.bufferData(l.ARRAY_BUFFER,new Float32Array(k.matrix[1]),l.STATIC_DRAW);l.vertexAttribPointer(o+1,4,l.FLOAT,false,0,0);l.bindBuffer(l.ARRAY_BUFFER,h);l.bufferData(l.ARRAY_BUFFER,new Float32Array(k.matrix[2]),l.STATIC_DRAW);l.vertexAttribPointer(o+2,4,l.FLOAT,false,0,0);l.bindBuffer(l.ARRAY_BUFFER,f);l.bufferData(l.ARRAY_BUFFER,new Float32Array(k.matrix[3]),l.STATIC_DRAW);l.vertexAttribPointer(o+3,4,l.FLOAT,false,0,0)};nucleo.DashEngine.prototype._renderSolid=function(){var c=nucleo.Actor.MODE;var f=this.silos[c.SOLID];
var h=f.data;var a=this.renderer;var e=a.transforms;var d=a.pm;var j=a.gl;var n=this.essl;d.setUniform("uUseShading",true);d.enableAttribute(n.VERTEX_ATTRIBUTE);d.enableAttribute(n.NORMAL_ATTRIBUTE);d.enableAttribute(n.COLOR_ATTRIBUTE);this.enableMatrixAttribute(n.ACTOR_MATRIX);var l=this._gl_buffers[c.SOLID].mixin;j.bindBuffer(j.ARRAY_BUFFER,l);j.bufferData(j.ARRAY_BUFFER,new Float32Array(h.mixin),j.STATIC_DRAW);d.setAttributePointer(n.VERTEX_ATTRIBUTE,3,j.FLOAT,false,36,0);d.setAttributePointer(n.NORMAL_ATTRIBUTE,3,j.FLOAT,false,36,12);
d.setAttributePointer(n.COLOR_ATTRIBUTE,3,j.FLOAT,false,36,24);var k=this._gl_buffers[c.SOLID].index;j.bindBuffer(j.ELEMENT_ARRAY_BUFFER,k);j.bufferData(j.ELEMENT_ARRAY_BUFFER,new Uint16Array(h.index),j.STATIC_DRAW);j.drawElements(j.TRIANGLES,h.index.length,j.UNSIGNED_SHORT,0)};